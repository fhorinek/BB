...
{
	tft_dma_done = true;
	vTaskNotifyGiveFromISR((TaskHandle_t)GUIHandle, NULL);
}
...
static void tft_init_table(const uint8_t *table, uint16_t size)
{
    for (uint8_t i = 0; i < size; i += 2)
	{
        ...
        else
        {
        	...
        	//DBG("writing %04X %04X -> %04X", cmd, data, tft_read_register(cmd));
        }
    }
}
...
void tft_init_display()
{
	...
	tft_set_rotation(2);             //INVERTED PORTRAIT
