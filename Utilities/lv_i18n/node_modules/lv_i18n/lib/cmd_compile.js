...
const { getIDX, getRAW }         = require('./compiler_template');
...
`);
  ...
  translationKeys.phrases.forEach(p => {
      if (!_.isPlainObject(p.value) && !data.singularKeys.includes(p.key) ) {
  ...
  let raw = getRAW(sorted_locales, data);

  if (args.output_raw) {
    writeFileSync(args.output_raw, raw);
    writeFileSync(args.output_raw + "-idx.h", idx);
  }
