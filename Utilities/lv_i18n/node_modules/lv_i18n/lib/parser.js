...
/*  let s_re = create_singular_re(opts.singularName);
  let s2_re = create_singular_re(opts.singularNameID);*/
  let s3_re = create_singular_re(opts.singularID);
//  let p_re = create_plural_re(opts.pluralName);

/*  let singulars = extract(text, s_re).map(o => Object.assign(o, { plural:  false }));
  let singulars2 = extract(text, s2_re).map(o => Object.assign(o, { plural:  false }));*/
  let singulars3 = extract(text, s3_re).map(o => Object.assign(o, { plural:  false }));
  
  //let plurals = extract(text, p_re).map(o => Object.assign(o, { plural:  true }));

  //singulars = singulars.concat(singulars2);
  //singulars = singulars.concat(singulars3);
  let singulars = singulars3

  console.log(text);
  console.log(singulars);

  //return _.sortBy(singulars.concat(plurals), 'line');
  return _.sortBy(singulars, 'line');
};
