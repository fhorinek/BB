...
#define Fuel_ModelCfg_val 0x8004    //charge to 4,2V, high side sensing
...
{
	ASSERT(
			HAL_I2C_Mem_Write(&sys_i2c, MAX_ADR, reg, I2C_MEMADD_SIZE_8BIT, &data, 1, 100) == HAL_OK);
}

static void write_reg16(uint8_t reg, uint16_t data)
{
	ASSERT(
			HAL_I2C_Mem_Write(&sys_i2c, MAX_ADR, reg, I2C_MEMADD_SIZE_8BIT, (uint8_t *)&data, 2, 100) == HAL_OK);
}
...
static uint8_t read_reg(uint8_t reg)
{
	...
	ASSERT(
			HAL_I2C_Mem_Read(&sys_i2c, MAX_ADR, reg, I2C_MEMADD_SIZE_8BIT, &data, 1, 100) == HAL_OK);
...
static uint16_t read_reg16(uint8_t reg)
{
	...
	ASSERT(
			HAL_I2C_Mem_Read(&sys_i2c, MAX_ADR, reg, I2C_MEMADD_SIZE_8BIT, (uint8_t *)&data, 2, 100) == HAL_OK);
