...
void fanet_init()
{
	...
	GpioWrite(FANET_RESET, LOW);
	GpioWrite(FANET_SW_EN, HIGH);
	osDelay(10);
	GpioWrite(FANET_RESET, HIGH);
}
...
void fanet_transmit_message(uint8_t type, fanet_addr_t dest, uint8_t len, uint8_t * payload, bool forward = false, bool ack_required = false)
{
	char line[128];
...
void fanet_step()
{
	...
	if (next_transmit_name <= HAL_GetTick() && fc.fanet.valid && fc.gnss.valid && config_get_bool(&config.devices.fanet.broadcast_name))
	{
		...
		fanet_transmit_message(FANET_MSG_TYPE_NAME, dest, strlen(name), (uint8_t *)name);
	}
}
