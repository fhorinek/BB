...
//gui page global memory
uint8_t gui_page_memory[GUI_PAGE_MEMORY_SIZE];
...
void gui_set_backlight(uint8_t val)
{
	...
	__HAL_TIM_SET_COMPARE(&tim_rgb_and_bck, tim_bckl_channel, val);
}
...
void gui_loop()
{
	...
	if (gui_task_next != NULL)
	{
		...
		//remove input group from old page
		...
		switch (gui_task_next_anim)
		{
			...
			case(GUI_SW_TOP_BOTTOM):
				...
				lv_anim_set_ready_cb(lv_last_anim, gui_switch_cb);
				break;
