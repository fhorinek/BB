...
REDSTATUS RedOsMutexInit(void)
{
    ...
    UINT state = tx_semaphore_create(&xMutex, "red_semaphore", 0);
    tx_semaphore_put(&xMutex);
...
{
    tx_semaphore_get(&xMutex, UX_WAIT_FOREVER);
}
