...
void setting_cb(lv_obj_t * obj, lv_event_t event, uint8_t index)
{
	...
	if (event == LV_EVENT_CLICKED)
	{
		...
		if (index == 1)
			gui_switch_task(&gui_vario, LV_SCR_LOAD_ANIM_MOVE_LEFT);
...
lv_obj_t * settings_init(lv_obj_t * par)
{
	...
	gui_list_text_add_entry(list, "Pilot");
	gui_list_text_add_entry(list, "Vario");
	gui_list_text_add_entry(list, "GNSS");
	gui_list_text_add_entry(list, "FANET");
    gui_list_text_add_entry(list, "Audio");
