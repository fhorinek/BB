...
lv_obj_t * display_init(lv_obj_t * par)
{
	...
	gui_list_auto_entry(list, _h("Language"), &config.display.language, NULL);
	gui_list_auto_entry(list, _h("Brightness"), &config.display.backlight, &back_param);
	gui_list_auto_entry(list, _h("Page animation"), &config.display.page_anim, NULL);

	if (pwr.fuel_gauge.status == fc_dev_ready)
		gui_list_auto_entry(list, _h("Battery percent"), &config.display.bat_per, NULL);
