...
lv_obj_t * calib_menu_init(lv_obj_t * par)
{
    ...
    if (fc.imu.status == fc_dev_ready)
    {
		gui_list_auto_entry(list, "Recalibrate Gyroscope", CUSTOM_CB, calib_menu_calib_gyro);
		gui_list_auto_entry(list, "Recalibrate Accelerometer", CUSTOM_CB, calib_menu_calib_acc);
		gui_list_auto_entry(list, "Recalibrate Magnetometer", CUSTOM_CB, calib_menu_calib_mag);
		gui_list_auto_entry(list, "Reset calibration", CUSTOM_CB, calib_menu_calib_clear);
	}
