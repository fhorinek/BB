...
typedef enum period {
	...
	PERIOD_First = PERIOD_ALL_YEARS, PERIOD_Last = PERIOD_DAYS_IN_MONTH
} period_t;
...
typedef enum datatype {
	...
	DATATYPE_First = DATATYPE_HOURS, DATATYPE_Last = DATATYPE_ODO
} datatype_t;
...
#define BAR_WIDTH 140    // the maximum width of the bar
...
static void read_dir(char *path)
{
	...
	if (dir != NULL)
	{
		while ( (entry = red_readdir(dir)) )
		{
			...
			char full_path[REDCONF_NAME_MAX];
...
static void summarize_data()
{
	...
	for ( i = 0; i < ROW_NUM; i++ )
	{
		...
		case PERIOD_DAYS_IN_MONTH:
			...
			break;
		case PERIOD_ALL_YEARS:
			sprintf(label_t, "%d", local->year_min + i);
			break;
		}
...
void flightbook_statistics_load_task(void * param)
{
	...
	for ( i = 0; i < STATS_NUM; i++ )
		if ( local->stats[i].year_since_70 == 0) break;
    if ( i >= STATS_NUM )
    {
...
void flightbook_statistics_loop()
{
    ...
    if ( local->recalculate ) {
    	...
    	{
    	case PERIOD_ALL_YEARS:
    		strcpy(text, LV_SYMBOL_RIGHT " Year");
    		sprintf(period_title, "all");
    		break;
    	case PERIOD_MONTHES_IN_YEAR:
    		strcpy(text, LV_SYMBOL_RIGHT " Days");
    		sprintf(period_title, "%d", local->year);
    		break;
    	case PERIOD_DAYS_IN_MONTH:
    		strcpy(text, LV_SYMBOL_RIGHT " All");
    		sprintf(period_title, "%d/%02d", local->year, local->month);
    		break;
    	}

    	lv_label_set_text(local->label_period, text);
    	lv_obj_align(local->label_period, local->par, LV_ALIGN_IN_BOTTOM_RIGHT, 0,0);

    	switch(local->datatype)
    	{
    	case DATATYPE_HOURS:
    		strcpy(text, LV_SYMBOL_RIGHT " Num");
        	sprintf(title_text, "Hours in %s", period_title);
    		break;
    	case DATATYPE_NUM:
    		strcpy(text, LV_SYMBOL_RIGHT " Odo");
        	sprintf(title_text, "Flights in %s", period_title);
    		break;
    	case DATATYPE_ODO:
    		strcpy(text, LV_SYMBOL_RIGHT " Hours");
        	sprintf(title_text, "Distance in %s", period_title);
    		break;
    	}
    	lv_label_set_text(local->label_type, text);
