    ...
    BDEVINFO               *pInfo)
{
    ...
    /*  Note: the actual card block size is sdCardInfo.CardBlockSize, but the
        ...
        512-byte sectors.
    */
    pInfo->ulSectorSize = sdCardInfo.BlockSize;
...
static REDSTATUS CheckStatus(void)
{
    ...
    HAL_SD_StateTypeDef transferState;

    do
    {
        transferState = HAL_SD_GetState(&hsd1);
        ulTimeout--;
    } while((transferState == HAL_SD_STATE_BUSY) && (ulTimeout > 0U));

    if(transferState != HAL_SD_STATE_READY)
    {
        redStat = -RED_EIO;
    }
