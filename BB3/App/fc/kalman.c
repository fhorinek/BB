...
#define Q_accel     30.0f
#define R_altitude  4.0f

#define dt 0.01f
#define _dtdt (dt * dt)

static float kalman_h;
static float kalman_v;
...
void kalman_propagate(float acceleration)
{
    ...
    kalman_h += kalman_v * dt + 0.5f * acceleration * _dtdt;
    kalman_v += acceleration * dt;
...
{
    kalman_update(altitude);
    kalman_propagate(accel);
